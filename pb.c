...
static PyObject* py_deviceapps_xwrite_pb(PyObject* self, PyObject* args) {
    size_t i;
    const char *device_id = NULL, *device_type = NULL;
    const char *path;
    void *buf = NULL;
    ...
    if ((py_type = PyDict_GetItemString(py_device, "type"))){
        device_type = PyUnicode_AsUTF8(py_type);
        if (!device_type) {
            goto error;
        }
        // Set device type
    ...
    if ((py_id = PyDict_GetItemString(py_device, "id"))){
        device_id = PyUnicode_AsUTF8(py_id);
        if (!device_id) {
            goto error;
        }
    ...
    if((py_apps = PyDict_GetItemString(next_message, "apps"))){
        if (!PyList_Check(py_apps)) {
            ...
            for(i=0; i<msg.n_apps; i++){
               ...
               msg.apps[i] = (uint32_t) PyLong_AsLong(py_app); 
               if(msg.apps[i] == (uint32_t) -1 && PyErr_Occurred()){
                   goto error;
               }
            }
    }
    ...
    for(i=0; i<msg.n_apps; i++){
       msg.apps[i] = (uint32_t) PyLong_AsLong(PyList_GetItem(py_apps, i)); 
    }
...
//      (void) Py_InitModule3("pb", PBMethods);
// }

static char py_deviceapps_xwrite_pb_docs[] =
    "";

static char py_deviceapps_xread_pb_docs[] =
    "";

static PyMethodDef module_methods[] = {
    {"deviceapps_xwrite_pb", (PyCFunction) py_deviceapps_xwrite_pb, 
     METH_VARARGS, py_deviceapps_xwrite_pb_docs},
    {"deviceapps_xread_pb", (PyCFunction) py_deviceapps_xread_pb, 
     METH_VARARGS, py_deviceapps_xread_pb_docs},
    {NULL, NULL, 0, NULL}
};

static struct PyModuleDef PBMethods =
{
...
{
    return PyModule_Create(&PBMethods);
}